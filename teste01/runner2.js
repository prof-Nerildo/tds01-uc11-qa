...
function test(descricao, fn){
    ...
    try {
        ...
        resultadoDetalhe = fn(); // <--- ATUALIZADO: Armazena o retorno
    ...
    // Verifica se a função de teste retornou um elemento HTML (como o 'detalhesLista')
    if (resultadoDetalhe instanceof HTMLElement) {
        li.appendChild(resultadoDetalhe); // Anexa o elemento retornado (a lista de detalhes)
    }
...
test('deve aplciar 10% de desconto corretamente para todos os cenários aleatórios ', () => {
    const NUMERO_DE_TESTE = 10; // Vamos "martelar!" a função 100 vezes!!! :D

    const detalhesLista = document.createElement('ul');
    detalhesLista.className = 'lista-detalhe';
    ...
    for( let i = 0; i < NUMERO_DE_TESTE; i++ ){
        ...
        liDetalhe.textContent = `
            [Cenário ${i + 1}] OK - 
            Total: R$ ${totalBruto.toFixed(2)} -->
            Com 10% de Desconto: R$ ${totalRecebido.toFixed(2)}
        `;
        detalhesLista.appendChild(liDetalhe);
    }
